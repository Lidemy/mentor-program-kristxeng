...
$(function(){
	...
	$('.container-fluid').on('click', e =>{
		...
		//處理子留言框折疊或顯示的toggle
		if( $(e.target).hasClass('sub-cmmt__collapse-toggle') ){
		...
		//編輯完成按鍵處理
		if( $(e.target).hasClass('cmmt__edited') ){
			...
			if( chkRequired(content) ){
						...
						if( req.responseText === 'modified' ){
							...
							content.outerHTML = `
								<div class="cmmt__content">
									${htmlspecialchars(content.value.replace(/<script>|<\/script>/gi, "[removed]"))}
								</div>`
							e.target.innerText = '編輯';
							e.target.className = 'cmmt__edit cmmt__link';
				...
				req.open( 'POST', '/ci-cmmt/comment/modify/', true );
		...
		//編輯留言按鍵處理
		if( $(e.target).hasClass('cmmt__edit') ){
			var content = e.target.parentNode.parentNode.parentNode.nextElementSibling;
			...
			e.target.className = 'cmmt__edited cmmt__link';
		...
		//刪除留言按鍵處理
		if( $(e.target).hasClass('cmmt__delete') ){
			var content = e.target.parentNode.parentNode.parentNode.nextElementSibling;
			var cmmt_id = content.nextElementSibling;
			...
			if( chkRequired(content) ){
				...
				req.open( 'POST', '/ci-cmmt/comment/delete/', true );
				req.setRequestHeader( 'Content-type', 'application/x-www-form-urlencoded' );
	...
	$('.cmmt__logout').on('click', ()=>{
		...
		document.location.href = '/ci-cmmt/user/logout/';
		
	})

	//新增留言的 ajax 處理
	$('.container-fluid').on('click','.cmmt__btn[type="submit"]' , function(e){
		...
		if( $(e.target).prev().prev().val() === '' ){
			...
			$.ajax({
				...
				success: res =>{
					...
					var content = (htmlspecialchars( $(e.target).prev().prev().val()).replace(/<script>|<\/script>/gi, "[removed]") );
					...
					//顯示在主留言
					if( e.target.className === 'cmmt__btn btn btn-primary'){
								...
								<div class="cmmt__header">
									...
									<div>
										...
										<div class="cmmt__edit-delete">
											<span class="cmmt__edit cmmt__link">編輯</span>&nbsp;/&nbsp;<span class="cmmt__delete cmmt__link">刪除</span>
										</div>
									</div>
								</div>
							...
							<div class="sub-cmmt col-11">
								<div class="cmmt__header">
									...
									<div>	
										<div class="cmmt__time">${created_by}</div>
										<div class="cmmt__edit-delete">
											<span class="cmmt__edit cmmt__link">編輯</span>&nbsp;/&nbsp;<span class="cmmt__delete cmmt__link">刪除</span>									</div>
									</div>
								</div>
