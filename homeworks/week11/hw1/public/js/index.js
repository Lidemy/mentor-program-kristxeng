	...
	$('.container-fluid').on('click', e =>{
		...
		if( $(e.target).hasClass('cmmt__edited') ){
			...
			if( chkRequired(content) ){
				...
				req.open( 'POST', '/modify_cmmt', true );
		...
		if( $(e.target).hasClass('cmmt__delete') ){
			...
			if( chkRequired(content) ){
				...
				req.open( 'DELETE', '/delete_cmmt', true );
				req.setRequestHeader( 'Content-type', 'application/x-www-form-urlencoded' );
		...
		if( $(e.target).prev().prev().val() === '' ){
			...
			$.ajax({
				...
				success: res =>{
					...
					var created_by = res.created_by;
					var content = htmlspecialchars( $(e.target).prev().prev().val() );
					var cmmt_id = res.cmmt_id;
